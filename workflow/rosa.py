"""Timeloop-backed ROSA reproduction workflow.
...
class RosaWorkflow:
    ...
    def write_combined_csv(
        ...
        return write_results_csv(results, path, group_components=True)
...
def write_results_csv(
    ...
    *,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    rows = [_result_row(result, group_components=group_components) for result in results]
...
def _result_row(result: SimulationResult, *, group_components: bool) -> Dict[str, object]:
    row = {
    ...
    if group_components:
        component_names = _grouped_component_values(result)
    else:
    ...
    for component in sorted(component_names):
        row[f"{component}_energy"] = _component_value(
            result, component, "energy", group_components
        )
        row[f"{component}_area"] = _component_value(
            result, component, "area", group_components
        )
        row[f"{component}_power"] = _component_value(
            result, component, "power", group_components
        )
    return row
...
def _grouped_component_values(result: SimulationResult) -> Tuple[str, ...]:
    ...
    grouped = [name for name, names in COMPONENT_GROUPS.items() if raw_components & set(names)]
...
def _default_workspace_dir(repo_root: Path) -> Path:
    package_workspace = Path(__file__).resolve().parents[1] / "workspace"
